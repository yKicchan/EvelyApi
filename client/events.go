...
func (c *Client) NewNearbyEventsRequest(ctx context.Context, path string, lat float64, lng float64, range_ int, limit *int, offset *int) (*http.Request, error) {
	...
	u := url.URL{Host: c.Host, Scheme: scheme, Path: path}
	values := u.Query()
	tmp24 := strconv.FormatFloat(lat, 'f', -1, 64)
	values.Set("lat", tmp24)
	tmp25 := strconv.FormatFloat(lng, 'f', -1, 64)
	values.Set("lng", tmp25)
	tmp26 := strconv.Itoa(range_)
	values.Set("range", tmp26)
	if limit != nil {
