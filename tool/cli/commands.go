...
type (
	...
	// MyListEventsCommand is the command line data structure for the my_list action of events
	MyListEventsCommand struct {
		// 取得件数
		Limit int
		// 除外件数
		Offset      int
	...
	// ModifyUsersCommand is the command line data structure for the modify action of users
	ModifyUsersCommand struct {
		Payload     string
		ContentType string
		PrettyPrint bool
	}

	// ModifyTokenUsersCommand is the command line data structure for the modify_token action of users
	ModifyTokenUsersCommand struct {
		Payload     string
		ContentType string
		PrettyPrint bool
	}

	// SettingUsersCommand is the command line data structure for the setting action of users
	SettingUsersCommand struct {
		Payload     string
		ContentType string
		PrettyPrint bool
	}
...

{
   ...
   "url": "http://comp.ecc.ac.jp/"
	...
	command.AddCommand(sub)
	tmp7 := new(ModifyUsersCommand)
	sub = &cobra.Command{
		Use:   `users ["/api/develop/v2/users"]`,
		Short: ``,
		Long: `
...

{
   ...
   "tel": "090-1234-5678"
	...
	command = &cobra.Command{
		Use: "modify-token",
		Short: `インスタンスIDの登録・更新
認証ありで登録ユーザーを、認証なしでゲストユーザーを登録・更新する`,
	}
	tmp8 := new(ModifyTokenUsersCommand)
	sub = &cobra.Command{
...

{
   ...
   "instance_id": "id"
	...
	command = &cobra.Command{
		Use:   "my-list",
		Short: `自分のイベント一覧を取得する`,
	}
	tmp9 := new(MyListEventsCommand)
	sub = &cobra.Command{
		Use:   `events ["/api/develop/v2/events/my_list"]`,
		Short: ``,
	...
	tmp11 := new(NotifyEventsCommand)
	sub = &cobra.Command{
		Use:   `events ["/api/develop/v2/events/notify"]`,
		Short: ``,
		Long: `
...

{
   "email": "yKicchanApp@gmail.com"
	...
	tmp16 := new(SettingUsersCommand)
	sub = &cobra.Command{
		Use:   `users ["/api/develop/v2/users/setting"]`,
		Short: ``,
		Long: `
...

{
   ...
   "tel": "090-1234-5678"
	...
	tmp23 := new(VerifyTokenAuthCommand)
	sub = &cobra.Command{
...
// Run makes the HTTP request corresponding to the MyListEventsCommand command.
func (cmd *MyListEventsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
		path = "/api/develop/v2/events/my_list"
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.MyListEvents(ctx, path, intFlagVal("limit", cmd.Limit), intFlagVal("offset", cmd.Offset))
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *MyListEventsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	cc.Flags().IntVar(&cmd.Limit, "limit", 10, `取得件数`)
	var offset int
	cc.Flags().IntVar(&cmd.Offset, "offset", offset, `除外件数`)
}
...
func (cmd *NotifyEventsCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = "/api/develop/v2/events/notify"
	}
	var payload client.NotifyPayload
	if cmd.Payload != "" {
		err := json.Unmarshal([]byte(cmd.Payload), &payload)
		if err != nil {
			return fmt.Errorf("failed to deserialize payload: %s", err)
...
// Run makes the HTTP request corresponding to the ModifyUsersCommand command.
func (cmd *ModifyUsersCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
		path = "/api/develop/v2/users"
	}
	var payload client.UserModifyPayload
	if cmd.Payload != "" {
		err := json.Unmarshal([]byte(cmd.Payload), &payload)
		if err != nil {
			return fmt.Errorf("failed to deserialize payload: %s", err)
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ModifyUsers(ctx, path, &payload, cmd.ContentType)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *ModifyUsersCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	cc.Flags().StringVar(&cmd.Payload, "payload", "", "Request body encoded in JSON")
	cc.Flags().StringVar(&cmd.ContentType, "content", "", "Request content type override, e.g. 'application/x-www-form-urlencoded'")
}

// Run makes the HTTP request corresponding to the ModifyTokenUsersCommand command.
func (cmd *ModifyTokenUsersCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.ModifyTokenUsers(ctx, path, &payload, cmd.ContentType)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *ModifyTokenUsersCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	cc.Flags().StringVar(&cmd.Payload, "payload", "", "Request body encoded in JSON")
	cc.Flags().StringVar(&cmd.ContentType, "content", "", "Request content type override, e.g. 'application/x-www-form-urlencoded'")
}

// Run makes the HTTP request corresponding to the SettingUsersCommand command.
func (cmd *SettingUsersCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
		path = "/api/develop/v2/users/setting"
	}
	var payload client.SettingPayload
	if cmd.Payload != "" {
		err := json.Unmarshal([]byte(cmd.Payload), &payload)
		if err != nil {
			return fmt.Errorf("failed to deserialize payload: %s", err)
	...
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.SettingUsers(ctx, path, &payload, cmd.ContentType)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *SettingUsersCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	cc.Flags().StringVar(&cmd.Payload, "payload", "", "Request body encoded in JSON")
	cc.Flags().StringVar(&cmd.ContentType, "content", "", "Request content type override, e.g. 'application/x-www-form-urlencoded'")
}
